...
Немного затем эту часть переписал, внес playbook в конфиг Vagrant


С чем пришлось столкнуться.


Поменять путь в сервисе tftpd (учтено в ansible) - так как все настройки перенесены в каталог nginx

kernel-panic на клиенте, есkи количество процессоров меньше 2. связано с особенностями виртуализации на моей системе

По label Auto install NFS - смешанный режим (чтобы показать обе части задания) - Файл kickstart берется http, образ по nfs

Итоги установки системы на скриншотах в папке files.
